...
function Register({ onRegistered, onNavigate }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!err.response) {
        ...
        setError(err.response.data?.message || `Error ${err.response.status}: No se pudo crear la cuenta.`)
      }
    } finally {
      setLoading(false)
    }
      ...
      >
        ...
        >
          <Box sx={{
            ...
            <PersonAddIcon sx={{ color: 'primary.main', fontSize: 26 }} />
          </Box>
        </motion.div>
        ...
        <Box component="form" onSubmit={handleSubmit} sx={{ display: 'flex', flexDirection: 'column', gap: 2.5 }}>
          {[
            ...
            { label: 'Contraseña',         name: 'password', type: 'password', placeholder: 'Mínimo 6 caracteres', minLength: 6 },
          ].map((field, i) => (
            <motion.div key={field.name} custom={i} variants={fieldVariants} initial="hidden" animate="show">
              <TextField
                ...
                size="small"
                disabled={loading}
                inputProps={field.minLength ? { minLength: field.minLength } : undefined}
              />
            </motion.div>
          ))}
